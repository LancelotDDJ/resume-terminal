...
$("#stage").innerHTML = `
  ...
  <section id="boot" class="boot" aria-label="系统启动">
    ...
    <div class="welcome"><div class="welcome-panel"></div><div class="welcome-heading">WELCOME TO</div><div class="welcome-company"><strong>DONG DEJIA</strong><strong class="welcome-highlight" aria-hidden="true">DONG DEJIA</strong></div><div class="welcome-database">PERSONAL ARCHIVE</div><div class="welcome-logo">${logo}</div></div>
  </section>
  ...
  <footer class="system-footer"><span><i class="status-light"></i> SESSION AUTHORIZED</span><span>DONG DEJIA <i>／</i> <span id="clock">00:00:00</span></span></footer>
  <div id="modal-root"></div><div id="toast" class="toast" role="status"></div>
  <div id="loading" class="loading"><div class="loading-mark">${logo}</div><span>LOADING PERSONAL ARCHIVE</span><i></i></div>
`;
...
if (reviewParams.get("review") === "1") {
  ...
  window.addEventListener("message", (event) => {
    if (
      ...
      event.data?.type !== "portfolio-review-frame"
    )
      return;
...
const saved = new Set<string>(readLocal<string[]>("ddj-saved", []));
const storedPrefs = readLocal<Partial<{ sound: boolean; music: boolean; soundVolume: number; musicVolume: number; reduced: boolean; quality: boolean; rendering: RenderQuality }>>("ddj-settings", {});
const prefs = {
  sound: true,
...
function saveAudioPrefs() {
  try {
    localStorage.setItem("ddj-settings", JSON.stringify(prefs));
  } catch {}
  audio.configure(prefs);
}
function savePrefs() {
...
function toggleSaved() {
  ...
  try {
    localStorage.setItem("ddj-saved", JSON.stringify([...saved]));
  } catch {}
  $("#saved-count").textContent = String(saved.size).padStart(2, "0");
...
function renderModal() {
  ...
  $("#modal-root").innerHTML =
    `<div class="modal-backdrop"><section class="terminal-modal ${modal === "settings" ? "settings-modal" : ""}" role="dialog" aria-modal="true" aria-label="${modal === "settings" ? "系统设置" : modal === "saved" ? "收藏档案" : "档案检索"}"><div class="modal-top"><span>DONG DEJIA / ${modal === "settings" ? "SYSTEM PREFERENCES" : "ARCHIVE DIRECTORY"}</span><button data-action="close-modal" aria-label="关闭窗口">CLOSE <span>×</span></button></div>${modal === "settings" ? settingsMarkup() : `<h2>${modal === "saved" ? "SAVED ARCHIVES" : "ARCHIVE INDEX"}<small>${modal === "saved" ? "收藏档案" : "个人档案检索"}</small></h2><div class="search-field"><span>⌕</span><input id="archive-search" type="search" autocomplete="off" placeholder="输入档案编号、名称或关键词" aria-label="检索档案"/><span class="key">ESC</span></div><div class="category-filters">${categories.map((c, i) => `<button data-filter="${escapeHtml(c)}" class="${i === 0 ? "active" : ""}">${escapeHtml(c)}</button>`).join("")}</div><div class="result-header"><span>FILE / 档案</span><span>ROLE / 角色</span><span>ACCESS</span></div><div id="search-results" class="search-results"></div><div class="modal-bottom"><span id="result-count"></span><span>PERSONAL ARCHIVE <i>●</i> CONNECTED</span></div>`}</section></div>`;
...
async function start() {
  try {
    ...
    if (matchMedia("(max-width: 820px)").matches) {
      setTimeout(
        () => notify("小屏设备建议横屏浏览，双指可缩放场景；桌面端体验最佳"),
        1200,
      );
    }
