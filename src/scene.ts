...
// Text meshes baked into the GLB by the original Blender scripts carry the
// original project's branding. They are dropped at load time; the dynamic
// canvas label (drawn from brand.ts) provides the on-model text instead.
const BAKED_TEXT_MESH = /^(Company label|Database label|Serial number|Information label|Symbol label|Edge inscription)/i;
...
export class ArchiveScene {
  ...
  async load(assetUrl = `${import.meta.env.BASE_URL}assets/archive-cassette.glb`) {
    ...
    gltf.scene.traverse((o) => {
      if (o instanceof THREE.Mesh && !isBakedText(o)) meshes.push(o);
    });
