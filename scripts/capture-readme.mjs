...
try {
  ...
  await page.waitForFunction(
    () => window.ddj?.stats().ready && !document.querySelector("#loading"),
    null,
    { timeout: 60000 },
  );
  ...
  await page.evaluate(() => window.ddj.select(0));
  await wait(1600);
  await record(
    "decryption",
    async () => {
  ...
  await page.waitForFunction(
    () => window.ddj.stats().decryption.phase === "clear",
  );
  await shot("detail");
  ...
  await wait(400);
  await page.locator("#archive-search").fill("旅行");
  await wait(500);
  await shot("search");
  await page.keyboard.press("Escape");
  await wait(350);
  ...
  manifest.stats = await page.evaluate(() => window.ddj.stats());
  await writeFile(
